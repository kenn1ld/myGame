...
private:
    ...
    class AStar {
    public:
        AStar(const GameWorld& world);
        std::list<sf::Vector2i> findPath(const sf::Vector2i& start, const sf::Vector2i& goal);
    ...
    private:
        struct Node {
            ...
            float cost;
            float heuristic;
            Node* parent;  // raw pointer since unique_ptr should not be used for this use-case
        ...
        NodeList getSuccessors(Node* node);
        bool contains(const NodeList& list, int x, int y);
        std::list<sf::Vector2i> reconstructPath(Node* node);
    };
